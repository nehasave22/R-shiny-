...
setwd("C:/Users/saven/OneDrive/Documents/Rshinyprojects/")
...
function(input, output) { # server is defined within these parentheses
  ...
  output$two.variables <- renderPlot({
    
      graphData <- passData()[,c(input$var.1,input$var.2)]
      
      theGraph <- ggplot(graphData) + geom_point(aes(x=graphData[,1], y=graphData[,2])) +
        xlab(input$var.1) +
        ylab(input$var.2) +
  ...
  output$summaryVar <- renderText({ 
    
    paste(names(summary(analytics[,"timeonsite"]))
          ,summary(analytics[,"timeonsite"]))
